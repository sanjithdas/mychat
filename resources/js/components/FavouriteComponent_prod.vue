...
<script>
    export default {
        ...
        methods:{
            save(){
                axios.post('/jobs/save/'+this.jobid).then(response=>this.show=true).catch(error=>alert('error'))

            },
            unsave(){

                axios.post('/jobs/unsave/'+this.jobid).then(response=>this.show=false).catch(error=>alert('error'))
